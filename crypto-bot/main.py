...
def set_secret(server_id, secret=None):
    conn = psycopg.connect(host=os.getenv('DB_HOST', 'localhost'), dbname=os.getenv('DB_NAME', 'postgres'), user=os.getenv('DB_USER', 'postgres'), password=os.getenv('DB_PASSWORD', 'postgres'), port=os.getenv('DB_PORT', 5432))
...
```'''
    ...
    embed.add_field(name="Other commands:", value="!setchannel [ticker, signal (optional)], !removealert, !setsecret [secret], !removesecret, !alerts, !togglealerts, !setprefix [prefix]", inline=False)
...
async def setsecret(ctx, secret=None):
    if not secret:
        ...
        return
    set_secret(ctx.guild.id, secret)
...
async def alert_request():
    print("✅ alert_request loop started.")
    while True:
        try: 
            conn = psycopg.connect(host=os.getenv('DB_HOST', 'localhost'), dbname=os.getenv('DB_NAME', 'postgres'), user=os.getenv('DB_USER', 'postgres'), password=os.getenv('DB_PASSWORD', 'postgres'), port=os.getenv('DB_PORT', 5432))
            alert = await q.get()
            ...
            if isinstance(alert, dict):
                ...
                if 'server_id' in alert and 'ticker' in alert and 'alert' in alert:
                    ...
                    with conn.cursor() as cur:
                        ...
                        cur.execute('SELECT alerts_on, secret FROM servers WHERE server_id = %s', (server_id,))
                        res = cur.fetchone()
                        if res is None:
                            logging.info("Server is not in server list.")
                            continue
                        else:
                            alerts_on = res[0]
